...
export class UniTradeProvider extends Dependency {
  ...
  public init = (web3: Web3) => {
    ...
    this.contract = new this.web3.eth.Contract(
      UniTrade.abi as AbiItem[],
      config.unitrade.address
    );
  };

  public listOrders = async () => {
    try {
      const activeOrdersLength = await this.contract.methods
        .getActiveOrdersLength()
        .call();
      const orders: IUniTradeOrder[] = [];
      for (let i = 0; i < activeOrdersLength; i += 1) {
        const activeOrderId = await this.contract.methods
          .getActiveOrderId(i)
          .call();
        const order = await this.contract.methods
          .getOrder(activeOrderId)
          .call();
        orders.push({
          ...order,
          orderId:
            typeof activeOrderId === "string"
              ? parseInt(activeOrderId)
              : activeOrderId,
        });
      }
      return orders;
    ...
    try {
      ...
      await this.contract.methods
        ...
        .on("receipt", function (receipt: any) {
          const profit = toBN(order.executorFee).sub(
            toBN(receipt.gasUsed).mul(toBN(gasPrice))
          );
